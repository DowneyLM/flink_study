package cn.avengers.app

import java.time.Duration
import java.util.Properties
import java.util.concurrent.TimeUnit

import cn.avengers.bean.{ClickLog, ClickLogWide, Message}
import cn.avengers.task.{ChannelHotTask, DataToWideTask}
...
object App {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val messageDSWithWatermark: DataStream[Message] = messageDS.assignTimestampsAndWatermarks(WatermarkStrategy.forBoundedOutOfOrderness[Message](
      Duration.ofSeconds(5)).withTimestampAssigner(new SerializableTimestampAssigner[Message] {
